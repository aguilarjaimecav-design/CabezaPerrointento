...
const dogHero = '/images/files_11301971-2026-08-16T19-03-54-679Z-image.png';
...
function Header() {
  ...
  return <>
    <div className="bg-primary-700 px-4 py-2 text-center text-[11px] font-semibold tracking-[0.16em] text-cream-100">ENTREGA A DOMICILIO EN SEVILLA · ENVÍO GRATIS DESDE {FREE_SHIPPING_THRESHOLD} €</div>
    <header className="sticky top-0 z-40 border-b border-cream-300/60 bg-cream-50/95 backdrop-blur-md">
      ...
      {mobile && <div className="border-t border-cream-300/60 bg-cream-50 px-5 pb-5 pt-3 xl:hidden"><nav className="grid gap-1">{navItems.map(([label, href]) => <Link key={label} to={href} onClick={() => { setMobile(false); scrollTop(); }} className="border-b border-cream-200 py-3 text-sm font-bold text-primary-700">{label}</Link>)}</nav></div>}
    </header>
  </>;
}

function Footer() {
  return <footer className="bg-primary-800 text-cream-100"><div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 sm:grid-cols-2 lg:grid-cols-[1.3fr_1fr_1fr_1fr] lg:px-8">
    ...
    <div><h3 className="footer-heading">Navegación</h3><div className="grid gap-3 text-sm text-cream-200"><Link to="/" onClick={scrollTop}>Inicio</Link><Link to="/tienda" onClick={scrollTop}>Tienda</Link><Link to="/tienda?especie=perro" onClick={scrollTop}>Perros</Link><Link to="/tienda?especie=gato" onClick={scrollTop}>Gatos</Link><Link to="/paseos-y-cuidados" onClick={scrollTop}>Paseos y cuidados</Link><Link to="/quienes-somos" onClick={scrollTop}>Quiénes somos</Link><Link to="/contacto" onClick={scrollTop}>Contacto</Link></div></div>
    <div><h3 className="footer-heading">Ayuda</h3><div className="grid gap-3 text-sm text-cream-200"><Link to="/contacto#faq" onClick={scrollTop}>Preguntas frecuentes</Link><Link to="/contacto#envios" onClick={scrollTop}>Envíos</Link><Link to="/contacto#devoluciones" onClick={scrollTop}>Devoluciones</Link><Link to="/contacto#privacidad" onClick={scrollTop}>Política de privacidad</Link><Link to="/contacto#terminos" onClick={scrollTop}>Términos y condiciones</Link></div></div>
    <div><h3 className="footer-heading">Estamos cerca</h3><div className="grid gap-4 text-sm text-cream-200"><a href="https://wa.me/34644789324" target="_blank" rel="noreferrer" className="flex gap-2"><MessageCircle size={17} className="shrink-0 text-accent-300" /> +34 644 789 324</a><a href="mailto:cabezaperro015@gmail.com" className="flex gap-2"><Mail size={17} className="shrink-0 text-accent-300" /> cabezaperro015@gmail.com</a><span className="flex gap-2"><MapPin size={17} className="shrink-0 text-accent-300" /> Sevilla</span></div></div>
...
function WhatsAppButton() { return <a href="https://wa.me/34644789324" target="_blank" rel="noreferrer" aria-label="Escribir por WhatsApp" className="fixed bottom-5 right-5 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-primary-700 text-cream-50 shadow-soft transition hover:-translate-y-1 hover:bg-primary-600"><MessageCircle size={25} /></a>; }
...
function Shop() {
  ...
  const brands = useMemo(() => { const set = new Set<string>(); allProducts.forEach((p) => set.add(p.marca)); return Array.from(set).sort(); }, [allProducts]);
...
function Checkout() {
  ...
  const calcularEnvio = async (e: ReactMouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (!result.disponible) { setEnvioDisponible(false); setEnvioError('Lo sentimos, no realizamos envíos a domicilio a destinos a más de 18 km. Contáctanos por WhatsApp para alternativas.'); setCalculando(false); return; }
      setEnvio(result.envio); setDistanciaKm(result.distancia_km); setCalculando(false);

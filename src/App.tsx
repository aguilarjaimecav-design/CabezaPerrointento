...
import { CATEGORIAS, categoriaNombre, fetchProductById, fetchProducts, fetchProductsByIds, fetchRelatedProducts } from '@/data/products';
...
import { SeparadorTriana, MapaSevilla, NaranjaIcon, AbanicoIcon, AzulejoStarIcon } from '@/components/Sevillana';
...
const ZEN_WHATSAPP_URL = `https://wa.me/34644789324?text=${encodeURIComponent('Buenas. Estoy interesado en adquirir un producto Zen del catálogo')}`;
function isZenProduct(product: Product): boolean { return product.nombre.toLowerCase().includes('zen'); }
...
function Footer() {
  return <footer className="bg-primary-800 text-cream-100 azulejo-bg-dark"><div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 sm:grid-cols-2 lg:grid-cols-[1.2fr_1fr_1fr_1fr] lg:px-8">
...
function Home() {
  ...
  useEffect(() => { fetchProductsByIds(['wonder-tails-adultos', 'fresh-mezcla-mediterranea-esterilizado', 'fresh-pollo-arroz-bajo-cereal']).then(setFeatured); }, []);
  return <div className="animate-fadeIn"><section className="hero-section"><div className="mx-auto grid max-w-7xl items-center gap-10 px-5 py-12 lg:grid-cols-[0.92fr_1.08fr] lg:px-8 lg:py-20"><div className="relative z-10"><p className="eyebrow text-cream-200">Alimentación · Paseos · Cuidados</p><h1 className="mt-5 max-w-xl font-serif text-5xl font-semibold leading-[1.05] text-cream-50 sm:text-6xl lg:text-7xl">Alimenta,<br /><em className="font-normal text-cream-300">cuida</em> y disfruta.</h1><p className="mt-7 max-w-lg text-base leading-7 text-cream-100 md:text-lg">Alimentación para perros y gatos, paseos y cuidados a domicilio en Sevilla. Todo lo que necesitan, con el cariño de siempre.</p><div className="mt-9 flex flex-wrap gap-3"><button onClick={() => { navigate('/tienda'); scrollTop(); }} className="button-light">Ver productos <ArrowRight size={17} /></button><button onClick={() => { navigate('/reservar'); scrollTop(); }} className="button-outline-light">Reservar un paseo</button></div><div className="mt-12 flex items-center gap-5 border-t border-cream-100/20 pt-5 text-xs text-cream-200"><span className="flex items-center gap-2"><MapPin size={15} /> Sevilla</span><span className="h-1 w-1 rounded-full bg-accent-400" /><span className="flex items-center gap-2"><ShieldCheck size={15} /> Trato cercano</span></div></div><div className="relative"><div className="relative mx-auto" style={{ maxWidth: '315px' }}><div className="absolute -inset-3 rounded-[2rem] border border-accent-400/40" /><div className="relative aspect-[9/16] max-h-[560px] overflow-hidden rounded-[1.7rem]"><video src="/images/0912.mp4" autoPlay loop muted playsInline className="h-full w-full object-cover" onError={(e) => { const v = e.currentTarget; v.style.display = 'none'; const img = document.createElement('img'); img.src = walkImage; img.alt = 'Cuidado con raíces'; img.className = 'h-full w-full object-cover'; v.parentElement?.appendChild(img); }} /><div className="absolute inset-0 bg-gradient-to-t from-primary-900/45 via-transparent to-transparent" /><div className="absolute bottom-6 left-6 rounded-xl bg-cream-50/95 px-4 py-3 shadow-soft"><p className="font-serif text-lg text-primary-800">Cuidado con raíces</p><p className="mt-0.5 text-xs text-secondary-600">Desde Sevilla, para ellos.</p></div></div><div className="absolute -bottom-5 -left-4 hidden h-20 w-20 items-center justify-center rounded-full border border-accent-400 bg-cream-100 text-center text-[10px] font-bold uppercase leading-4 tracking-wider text-primary-700 sm:flex"><NaranjaIcon size={16} className="text-accent-600 mb-0.5" />Hecho<br />en Sevilla</div></div></div></div></section>
    <section className="bg-primary-800 py-10 azulejo-bg-dark"><div className="mx-auto max-w-7xl px-5 lg:px-8"><TrustBar variant="dark" /></div></section>
    <section className="border-b border-cream-300/60 bg-cream-100 azulejo-bg"><div className="mx-auto grid max-w-7xl divide-y divide-cream-300/70 px-5 py-7 sm:grid-cols-3 sm:divide-x sm:divide-y-0 lg:px-8"><InfoMini icon={<Truck size={20} />} title="Te lo llevamos" text="En Sevilla" /><InfoMini icon={<Utensils size={20} />} title="Bien elegido" text="Productos de confianza" /><InfoMini icon={<MessageCircle size={20} />} title="Estamos cerca" text="Te atendemos por WhatsApp" /></div></section>
    <PromoBanner />
    <SeparadorTriana />
    <section className="section-space azulejo-bg"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><SectionIntro eyebrow="Para cada compañero" title="Lo que más se llevan" text="Alimentación pensada para cada etapa, cada gusto y cada forma de ser." /><Link to="/tienda" onClick={scrollTop} className="button-text shrink-0">Ver toda la tienda <ArrowRight size={16} /></Link></div><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{featured.map((p, i) => <ProductCard product={p} key={p.id} index={i} />)}</div></div></section>
    <SeparadorTriana variant="dark" />
    <section className="section-space"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="rounded-3xl bg-primary-700 p-8 text-cream-50 md:p-12 azulejo-bg-dark"><div className="grid items-center gap-8 lg:grid-cols-[1fr_auto]"><div><p className="eyebrow text-cream-300">Cuando lo necesitas</p><h2 className="mt-3 font-serif text-4xl md:text-5xl">Paseos y cuidados<br /><em className="font-normal text-cream-300">personalizados.</em></h2><p className="mt-5 max-w-xl leading-7 text-cream-100">Porque ellos también necesitan su momento. Conocemos sus rutinas, respetamos su forma de ser y les cuidamos como si fueran nuestros.</p></div><Link to="/paseos-y-cuidados" onClick={scrollTop} className="button-light">Descubrir servicios <ArrowRight size={17} /></Link></div></div></div></section>
    <SeparadorTriana />
    <section className="bg-cream-100 py-20 azulejo-bg"><div className="mx-auto grid max-w-7xl items-center gap-10 px-5 lg:grid-cols-[1fr_1fr] lg:px-8"><div className="overflow-hidden rounded-2xl"><img src={careImage} alt="Persona cuidando a su perro en casa" className="h-[440px] w-full object-cover" /></div><div className="max-w-xl"><SectionIntro eyebrow="Mucho más que una tienda" title="Ellos son uno más de la familia." text="En CabezaPerro creemos que cuidar bien empieza por conocerles. Por eso juntamos alimentación de calidad con servicios pensados para que estén bien, también cuando tú no puedes estar." /><div className="mt-8 grid gap-5 sm:grid-cols-3"><Value icon={<Heart size={19} />} title="Cercanía" /><Value icon={<AzulejoStarIcon size={19} className="text-accent-600" />} title="Calidad" /><Value icon={<ShieldCheck size={19} />} title="Confianza" /></div><Link to="/quienes-somos" onClick={scrollTop} className="button-dark mt-9">Conócenos <ArrowRight size={17} /></Link></div></div></section>
  </div>;
}

...
function Header() {
  ...
  return <>
    ...
    <header className="sticky top-0 z-40 border-b border-cream-300/60 bg-cream-50/95 backdrop-blur-md">
      ...
      {mobile && <><div className="fixed inset-0 z-50 bg-primary-900/80 overlay-enter xl:hidden" onClick={() => setMobile(false)} /><div className="fixed right-0 top-0 z-[60] flex h-full w-80 max-w-[85vw] flex-col bg-cream-50 shadow-soft drawer-enter xl:hidden"><div className="flex items-center justify-between border-b border-cream-300 px-5 py-4"><Brand /><button aria-label="Cerrar menú" onClick={() => setMobile(false)} className="icon-button"><X size={22} /></button></div><nav className="grid gap-1 px-5 py-4">{navItems.map(([label, href]) => <Link key={label} to={href} onClick={() => { setMobile(false); scrollTop(); }} className="border-b border-cream-200 py-3 text-sm font-bold text-primary-700 transition hover:text-accent-700">{label}</Link>)}</nav></div></>}
    </header>
  </>;
}

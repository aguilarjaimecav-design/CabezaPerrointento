...
function Shop() {
  ...
  const brands = useMemo(() => { const set = new Set<string>(); allProducts.filter((p) => species === 'todas' || p.especie === species).forEach((p) => set.add(p.marca)); return Array.from(set).sort(); }, [allProducts, species]);
  useEffect(() => { if (species === 'todas') return; const validCats = CATEGORIAS.filter((c) => c.especie === species || c.especie === 'ambas').map((c) => c.slug); if (category !== 'todas' && !validCats.includes(category as string)) setCategory('todas'); const validBrands = new Set(allProducts.filter((p) => p.especie === species).map((p) => p.marca)); if (brand !== 'todas' && !validBrands.has(brand)) setBrand('todas'); }, [species, category, brand, allProducts]);
